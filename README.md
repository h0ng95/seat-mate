...
`내 반에 앉아봐`는 친구들이 공유 링크로 참여해 12석 도트 교실을 채우는 Flutter Web 관계 콘텐츠다.

## 주요 기능

- 양력 생년월일과 선택 출생시간으로 계산하는 삼주/사주 원국
- 일간·일지·오행·합충 근거를 공개하는 결정론적 케미 지수
- 명리 관계 유형 10종과 2분단 x 3줄의 12석 짝꿍 배치
- stable hash로 만드는 성별 구분 없는 도트 캐릭터
- 링크를 받은 친구의 로그인 없는 참여
- 중복 참여, 정원 12명, 잘못된 링크 처리
- 문에서 확정 좌석까지 이동하는 짧은 애니메이션
- 우리 사이 결과 BottomSheet, 링크 공유, 클립보드 fallback
...
- `saju 0.1.1`의 절기 기반 원국 계산을 사용한다.
- 입력은 양력과 `Asia/Seoul` 표준시를 기준으로 한다.
- 출생시간이 있으면 년주·월주·일주·시주, 없으면 시주를 제외한 삼주만 사용한다.
- 궁합 점수는 일간 오행 30점, 일지 합충 25점, 오행 균형 25점, 전체 합충 20점의 합이다.
- 생성형 AI나 무작위 값은 원국 및 점수 산출에 관여하지 않는다.
- 원본 생년월일과 출생시간은 공개 view에서 제외하고 계산된 원국과 근거만 공유한다.

세부 규칙과 해석 한계는 [명리 궁합 계산 명세](docs/saju-compatibility.md)에 기록한다.
