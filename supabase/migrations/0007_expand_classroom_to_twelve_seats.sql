alter table public.classrooms
  drop constraint if exists classrooms_owner_seat_range,
  drop constraint if exists classrooms_member_count_range;

alter table public.classrooms
  add constraint classrooms_owner_seat_range
    check (owner_seat between 0 and 11),
  add constraint classrooms_member_count_range
    check (member_count between 1 and 12),
  alter column algorithm_version set default 3;

alter table public.classroom_members
  drop constraint if exists classroom_members_seat_range;

alter table public.classroom_members
  add constraint classroom_members_seat_range
    check (seat_index between 0 and 11),
  alter column algorithm_version set default 3;
...
create or replace function public.create_classroom(
  ...
  p_algorithm_version smallint default 3
)
...
begin
  ...
  if coalesce(cardinality(p_preferred_seats), 0) = 0
     or exists (
       ...
       where candidate not between 0 and 11
     ) then
