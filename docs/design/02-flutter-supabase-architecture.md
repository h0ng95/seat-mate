...
- `join_classroom`: 행 잠금, 중복/정원 검사, 최신 명단 검증, 전체 좌석 재배치, 구성원 저장

관계와 캐릭터 알고리즘은 앱과 DB 함수가 동일한 `algorithm_version`을 사용한다. 클라이언트는 방장을 제외한 전체 구성원의 최적 좌석을 계산해 RPC에 전달하고, DB는 교실 행을 잠근 뒤 최신 명단과 좌석 유일성을 검증해 한 트랜잭션으로 반영한다. 동시에 참여해 명단이 달라졌다면 클라이언트가 최신 교실을 다시 읽고 재계산한다.
